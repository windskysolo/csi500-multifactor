...
Layer 3  信号合成    src/signal/ridge_combiner.py（父类）
...
```
RidgeCombiner（src/signal/ridge_combiner.py）
...
| 方向 | 模块 | 关系 |
|------|------|------|
| 上游（父类）| `src/signal/ridge_combiner.py` | 继承 Walk-forward 骨架 |
...
**父类路径**：`src/signal/ridge_combiner.py`  
Ridge 的 expanding、rolling 与 decay 三种生产实现现已统一位于 `src/signal/`。`experiments/legacy` 中的同名旧入口仅作兼容转发，不再承载生产实现。
...
**EW-Ridge**（指数衰减加权 Ridge）在线性高斯假设下等价于以下三种框架（参见 `docs/历史归档/过程记录/2026年05月至06月工作记录/5.28/plans/time_varying_ridge_frameworks.md`）：
...
> **重要声明**：以上结论仅基于验证集（2021-2022）观察，仅用于指导下一步研究方向。权威账本当前记录测试集已用 1 次、剩余 2 次，且新运行仍处于锁定状态。
