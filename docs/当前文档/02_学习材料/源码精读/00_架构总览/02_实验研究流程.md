...
└──────────────────────────┬─────────────────────────────────────┘
                           ↓
         docs/历史归档/历史研究/历史改进记录/progress_log.md
         （确认历史诊断结论，避免重复排查）
                           ↓
...
测试集剩余运行次数的权威来源是 `docs/logs/test_set_runs.json`，不是 CLAUDE.md（CLAUDE.md 只是副本，容易漂移）。每次测试集运行后必须更新此文件，详见 `../07_实验流水线/06_测试集防护.md`。
