...
| run | 角色 | IR | 超额收益 | 最大回撤 |
|-----|------|----|---------|---------|
| baseline_expanding_ridge | 主基线 | 0.408 | +2.44% | -8.16% |
| challenger_rolling36 | 挑战者 | 0.966 | +5.80% | -8.38% |
| challenger_rolling48 | 挑战者 | **1.489** | +8.63% | -5.82% |
| challenger_rolling60 | 挑战者 | 1.176 | +6.87% | -6.73% |
| challenger_decay_ridge_hl24 | 挑战者（待实现） | — | — | — |
| challenger_decay_ridge_hl36 | 挑战者（待实现） | — | — | — |
| challenger_decay_ridge_hl48 | 挑战者（待实现） | — | — | — |

rolling48 在验证期指标最优，但还未晋升为主基线（需要人工审查后决定）。  
decay 系列挑战者需先实现 `src/signal/ridge_decay.py` 才能运行（当前会报 `NotImplementedError`）。
...
| 把 `decay_*` spec 跑起来（当前会报错） | `src/signal/ridge_decay.py` 还未实现 |
